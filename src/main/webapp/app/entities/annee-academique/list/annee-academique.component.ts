...
export class AnneeAcademiqueComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.anneeAcademiqueService
      ...
      .subscribe({
        next: (res: HttpResponse<IAnneeAcademique[]>) => {
  ...
  trackId(_index: number, item: IAnneeAcademique): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IAnneeAcademique[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/annee-academique'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
