...
export class SoutenanceComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.soutenanceService
      ...
      .subscribe({
        next: (res: HttpResponse<ISoutenance[]>) => {
  ...
  trackId(_index: number, item: ISoutenance): number {
    return item.id!;
  }

  delete(soutenance: ISoutenance): void {
    const modalRef = this.modalService.open(SoutenanceDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.soutenance = soutenance;
  ...
  protected onSuccess(data: ISoutenance[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
