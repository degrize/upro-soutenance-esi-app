...
export class EncadreurComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.encadreurService
      ...
      .subscribe({
        next: (res: HttpResponse<IEncadreur[]>) => {
  ...
  trackId(_index: number, item: IEncadreur): number {
    return item.id!;
  }

  delete(encadreur: IEncadreur): void {
    const modalRef = this.modalService.open(EncadreurDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.encadreur = encadreur;
  ...
  protected onSuccess(data: IEncadreur[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
