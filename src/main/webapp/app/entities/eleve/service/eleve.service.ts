...
export class EleveService {
  ...
  findAll(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IEleve[]>(this.resourceUrl + '/soutenance', { params: options, observe: 'response' });
  }
  ...
  protected convertDateFromClient(eleve: IEleve): IEleve {
    return Object.assign({}, eleve, {
      dateParcoursDebut: eleve.dateParcoursDebut?.isValid() ? eleve.dateParcoursDebut.format(DATE_FORMAT) : undefined,
      dateParcoursFin: eleve.dateParcoursFin?.isValid() ? eleve.dateParcoursFin.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateParcoursDebut = res.body.dateParcoursDebut ? dayjs(res.body.dateParcoursDebut) : undefined;
      res.body.dateParcoursFin = res.body.dateParcoursFin ? dayjs(res.body.dateParcoursFin) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((eleve: IEleve) => {
        eleve.dateParcoursDebut = eleve.dateParcoursDebut ? dayjs(eleve.dateParcoursDebut) : undefined;
        eleve.dateParcoursFin = eleve.dateParcoursFin ? dayjs(eleve.dateParcoursFin) : undefined;
      });
    }
    return res;
  }
